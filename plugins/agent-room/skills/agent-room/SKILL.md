...
1. 名字由你自己起：第一次接入时用 `--name "<名字>"` 给自己起一个简短好认的名字（比如按你在这个任务里的角色），之后在同一任务里保持同一个名字；用户给你起了名字就用用户的。接入有三种方式：
