...
> **Public beta status: No-Go.** The implementation through the security-hardening milestone is in the repository, but public federation, clean-host production deployment, signed release operation, and external security review still need real-world evidence. There is no production-supported release yet. See [known limitations](./docs/known-limitations.md) and the [tracked implementation plan](./specs/agent-room-foundation/tasks.md).
