...
- [x] 「接入 Agent」对话框：大厅工具栏、空房间提示与「本机 Agent」面板共用一个入口。选择宿主并一键配置，复制带固定 `sessionKey`、人物名和当前房间的指令，按该 `sessionKey` 轮询宿主会话并展示进入房间、失败码或断开状态；身份按设备与宿主持久化并记录账号归属，换账号不复用。浏览器端提示在运行 Agent 的电脑上完成并提供下载。Web 单元测试 11 项、Playwright 3 项覆盖，未接入真实宿主验收。
...
- 真实 Codex 任务已经读到生产房间的测试问题并生成回复，首次发送被自动发言权限检查拒绝；送达和持续接待尚未验收。修复、部署及实际回执进展单独记录在[真实 Codex 验收](./live-codex.md)。本机启动隔离 Bridge 的工具操作曾遭自动审批拒绝，未绕过该限制；契约测试不冒充真实模型调用。
...
- 这批功能已随 Alpha.27 发行，完整 CI、签名验证、生产部署及本机升级见[发布记录](./alpha27-release.md)。一键接入对话框已验证正式网页入口及引导，但仍未通过该入口完成真实宿主进入正式房间或后台唤醒验收；夹具与协议测试不代替这项验证。
