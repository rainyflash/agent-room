...
- 卷丢失或损坏（包括换机器恢复）时，网关自动重建，这个网络 Agent 的身份、名字、令牌、实例和收件箱都不变，只换一台 Matrix 设备：
  - 怎么认出来：恢复凭据在（身份与备份建好过）而存储目录没了；或 `matrix-adapter` 报设备与存储对不上（同一设备配新存储，上传一次性密钥时冲突）。
  1. 隔离旧存储：原文件移到存储目录下的恢复目录，不删。
  2. 实例换一台新的 Matrix 设备（`replace_instance_matrix_device`）：新设备 ID 是 `AR_<实例>_<后缀>`，先记到实例上，再在 Synapse 上删掉旧设备（连同它上传过的密钥）、为新设备签发会话；封存新的访问令牌。
     - 不沿用旧设备 ID：Synapse 删设备时不删这台设备的交叉签名，同一 ID 的新设备上传自签名时会被当成“已经有了”跳过，服务器上留的还是旧设备的签名，新设备永远算不上由主人签名。
     - 实例的 Matrix 设备以库里记的为准，网络 Agent 的会话从实例读出设备 ID。
  3. 打开新存储并完整同步一次，随后凭封存的恢复凭据恢复交叉签名，并从服务器端备份取回房间密钥。
  4. 其他成员仍认得这个身份，新设备由同一身份签名，首次见到即信任。
  - 同一个 Agent 一小时内最多重建一次，免得反复删设备；再对不上就记错误日志，回答暂时不可用。
...
- 私人房间对网络 Agent 沿用第 1 步的接纳规则：以 Agent 成员身份进入，主人（合成主体）不因此成为成员；房主可以移出。网络 Agent 停用后离开房间时，它在各私人房间的 Agent 成员自动记为已移出。
...
11. **3e-存储丢失重建**：存储丢失或设备与存储对不上时，隔离旧存储、实例换一台新的 Matrix 设备，凭封存的恢复凭据恢复。
...
- CI 的真实 Synapse 集成里加一条网络 Agent 全流程：创建、进大厅、收一条人的消息、回复、被本机 Bridge 验签通过、离开。
  ...
  - 私人房间一轮（第 3 步）：人在真实浏览器里建私人房间并生成口令（`apps/web/e2e-vertical/private-room-agent-code.e2e.ts`），本机 Agent 经 MCP 凭口令进来，网络 Agent 凭同一口令进来；本机 Agent 发加密消息、网络 Agent 解密后回复、本机 Agent 验签看到。之后重启控制面再来一轮；再停掉控制面、删掉网络 Agent 的存储目录、重启后再来一轮。口令与令牌都不得出现在日志里。网络 Agent 的存储放在纵向验收自己的目录（`.local/vertical/network-agents`），每轮清空。
...
- 2026-09-24：3e-存储丢失重建——认出存储丢了或与设备对不上时自动重建：隔离旧存储、同一台设备重新签发会话（不必新建实例）、凭封存的恢复凭据恢复身份与房间密钥；一小时内最多一次。真实 Synapse 上删掉存储目录后重建、恢复并在加密房间发言进了集成测试。
...
- 2026-09-25：3e 改为换一台新设备（新设备 ID，实例不变），不再同一台设备重新签发：集成测试在真实 Synapse 上发现，删掉设备后旧的交叉签名还留着，同一 ID 的新设备签不上，身份一直就绪不了。本机 Bridge 的冲突恢复仍是同一设备重签，有同样的隐患，另行跟进。
- 2026-09-25：3f-验收——真实环境验收加上私人房间一轮：凭口令进来、与本机 Agent 加密收发、控制面重启后与删掉存储重建后都照常。
- 2026-09-25：3f-验收第一轮发现：私人房间的正文授权只认“主人是能自动发送的成员”，凭口令进来的 Agent（主人不是成员）发加密正文被拒（403）。改为也认有效的 Agent 成员：能发言就能发正文、能查看就能读正文，不能以管理员身份查看。第 1 步起凭口令进来、主人不是成员的本机 Agent 同样受影响。
- 2026-09-25：3f-验收查出加密客户端的“同一位置重放”会卡死：长轮询超时、什么都没有时，服务器原样给回起点，下一次从同一位置同步就一直拿到同一个空结果，不再真的同步，收不到之后的消息和房间密钥，长轮询也空转。平时在线状态事件不断，把它掩住了；重建后在线状态发不出去，就露了出来。改为只重放往前走过的结果。
- 2026-09-25：第 3 步完成。真实环境验收（派发 `suite=all`）全程通过：凭口令进私人房间、与本机 Agent 加密收发、控制面重启后照常、删掉存储后重建照常。
- 2026-09-26：补上停用后的收尾：网络 Agent 停用后记下“已离开房间”时（自己 DELETE /me 或定时清理），同一条语句把它在私人房间的 Agent 成员记为已移出，房主不必再手动移出；之前已经离开的由迁移补记。
- 2026-09-26：第 3 步与停用后的收尾随 Alpha 52 发布（版本号已提升，等维护者发布）。
- 2026-09-26：本机 Bridge 的冲突恢复也改为换一台新设备：控制面新增 `POST /agent-instances/{id}/matrix-device`（复用 3e 的换设备用例），Bridge 恢复时改调它，并拒绝设备 ID 没变的响应。旧的同设备重签端点留给旧版 Bridge。
