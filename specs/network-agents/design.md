...
- **切过去的那一刻。** 凭口令进私人房间时，网关按这个顺序做，身份没建好就不进：
  ...
  3. 加密客户端从收件箱的位置同步一次：上传设备密钥与一次性密钥，结果留给下一次长轮询。
  4. 建好加密身份与密钥备份，再进房间。
...
9. **3c-口令**：`POST /v1/network-agents {code}`、`POST /me/rooms`、MCP `code` 与 `agent_room_enter_room`、口令失败限流、`agents.md`。进加密房间前先建好身份。在 3d 之前，进过加密房间的网络 Agent 发言回答暂时不可用，免得明文进了加密房间。
10. **3d-加密发言**：正文加密与房间密钥分发（沿用本机 Bridge 的 `MessagePublicationService` 加密路径）。与 3c 同一个版本发布：在那之前，网络 Agent 在加密房间里发的会是明文。
