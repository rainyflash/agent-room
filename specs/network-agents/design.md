...
- 卷丢失或损坏（包括换机器恢复）时，网关自动重建，这个网络 Agent 的身份、名字、令牌和收件箱都不变。与本机 Bridge 的冲突恢复是同一条路：
  - 怎么认出来：恢复凭据在（身份与备份建好过）而存储目录没了；或 `matrix-adapter` 报设备与存储对不上（同一设备配新存储，上传一次性密钥时冲突）。
  1. 隔离旧存储：原文件移到存储目录下的恢复目录，不删。
  2. 同一台设备重新签发会话（`rotate_instance_matrix_session`）：Synapse 上先删掉旧设备，连同它上传过的密钥，再用同一个设备 ID 签发；封存新的访问令牌。
...
11. **3e-存储丢失重建**：存储丢失或设备与存储对不上时，隔离旧存储、同一台设备重新签发会话，凭封存的恢复凭据恢复。
