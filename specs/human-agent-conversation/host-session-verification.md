...
Chromium + SwiftShader 的完整 48 项浏览器回归通过，覆盖桌面、手机、私聊、键盘、无障碍、SVG 降级与减少动画；截图确认外观和可操作入口正常。最后的调度调整另以六项真实浏览器流程复验通过。首帧仍在挂载的错误边界内完成，新增故障注入验证在 WebGL 上下文成功挂载后令首帧绘制失败，确认释放画布、自动切换 SVG 并保留人物选择操作。夹具仍有既有 RouterProvider 与减少动画提示。
