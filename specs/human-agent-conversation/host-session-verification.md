...
Chromium + SwiftShader 的完整 48 项浏览器回归通过，覆盖桌面、手机、私聊、键盘、无障碍、SVG 降级与减少动画；截图确认外观和可操作入口正常。最后的调度调整另以四项真实浏览器流程复验通过。夹具仍有既有 RouterProvider 与减少动画提示。
...
Linux CI 需对最终修复提交重新验收；本地软件渲染通过不能替代这一结果。
