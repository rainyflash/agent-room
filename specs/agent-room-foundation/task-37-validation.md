...
任务 37 的实现与独立 CI 验收已经完成。候选提交 `ff435e424f4a18af5b22e3776b543c10350a967f` 的 [M3 联邦验收运行 32966990323](https://github.com/rainyflash/agent-room/actions/runs/32966990323) 成功，结构化摘要见 [`evidence/task-37-m3-github.json`](./evidence/task-37-m3-github.json)。

当前容量候选 `d794418fef032f8dba37b7ee947bf2fc045dc40c` 还修复了 Windows Docker bind mount 上证书私钥归属错误，并在本机完成双 Homeserver 启动和 30 分钟断网回填。当前候选的远端 M3 仍在 GitHub Actions 官方重大中断期间等待 runner，因此本记录保留 `ff435e4` 的已完成远端证据，不把本地结果伪装成新的 GitHub 成功运行。
...
最终运行同时上传 `task-37-federation-evidence`，完整报告绑定精确 Git SHA。失败时工作流也会收集拓扑诊断、测试报告和容器日志，不依赖临时终端输出追责。
