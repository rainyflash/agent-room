...
> 状态：执行中；M0–M2 与 M3 任务 37–38 已完成，任务 39 等待不可压缩的 72 小时活跃 Bridge 证据
...
任务 15 的状态领域模型、签名 Matrix State Event、带抖动续租、房间可见性、本地离线派生和真实 Synapse 验收记录在 [任务 15 验证记录](./task-15-validation.md)。
...
任务 16 的主题目录、亲和力评分、软硬容量、事务预约、Matrix 加入补偿、可恢复自动建房、Space 挂载和真实依赖验收记录在 [任务 16 验证记录](./task-16-validation.md)。
...
任务 17 的幂等上传、私有对象存储、ClamAV 扫描、实时 Matrix 授权、短期读取票据、分布式下载限流、对象回收和真实依赖验收记录在 [任务 17 验证记录](./task-17-validation.md)。
...
任务 18 的预览协议、签名与幂等发送、未知提交对账、版本关系、持久化同步、实例验签和撤销时间窗验收记录在 [任务 18 验证记录](./task-18-validation.md)。

- [x] 19. 建立 Web 应用壳和会话状态机
...
任务 19 的真实 OIDC/Matrix 会话、刷新恢复、浏览器响应式验收和视觉保真账本记录在 [任务 19 验证记录](./task-19-validation.md)。
...
任务 20 的实时 Matrix 投影、200 节点帧预算、浏览器响应式验收和视觉账本记录在 [任务 20 验证记录](./task-20-validation.md)。

- [x] 21. 实现信号坞、正文检视器与发送器
...
任务 21 的统一信号模型、惰性正文闸门、诚实签名可信度、幂等发送状态机和浏览器响应式验收记录在 [任务 21 验证记录](./task-21-validation.md)。
...
任务 24 的真实浏览器登录、双 Bridge/MCP、Synapse 断网恢复、一次性交接、Canvas/列表降级、错误关联标识与日志扫描证据记录在 [任务 24 验证记录](./task-24-validation.md)。
...
任务 25 的领域聚合、PostgreSQL 权威成员事实、Matrix 安全边界、浏览器生命周期和真实依赖证据记录在 [任务 25 验证记录](./task-25-validation.md)。
...
任务 26 的会话聚合、PostgreSQL 幂等关系、Matrix DM、私有已读回执、屏蔽边界、沉浸式 Web 会话坞和真实依赖证据记录在 [任务 26 验证记录](./task-26-validation.md)。

- [x] 27. 实现 Matrix E2EE 与设备验证
...
任务 27 的强制 E2EE、官方 Matrix Crypto、设备信任/SAS、Secret Storage/密钥备份、客户端正文 AEAD、成员移除换钥和三设备真实恢复证据记录在 [任务 27 验证记录](./task-27-validation.md)。

- [x] 28. 实现多设备同步、恢复与设备管理
...
任务 28 的三类设备事实边界、Matrix Account Data 字段级收敛、产品设备/Agent 实例撤销、Matrix 清理重试、精确目标实例、全部本地设备丢失后的加密恢复和真实依赖证据记录在 [任务 28 验证记录](./task-28-validation.md)。
...
任务 29 的有界授权模型、每次发送的当前权限校验、PostgreSQL 原子消费、Bridge 副作用顺序、Automation Inspector、自动来源标记和真实依赖证据记录在 [任务 29 验证记录](./task-29-validation.md)。
...
任务 30 的本地立即屏蔽、服务端投递阻断、最小显式证据、房间权限矩阵、可逆 Matrix 治理、追加式审计和真实依赖证据记录在 [任务 30 验证记录](./task-30-validation.md)。

- [x] 31. 实现 Tauri 桌面壳与 Bridge 生命周期
...
任务 31 的窄化原生命令面、Bridge 生命周期监督、桌面专用构建、旧 Service Worker 退役、单实例、自动启动、托盘/通知/深链和真实 Windows 运行时证据记录在 [任务 31 验证记录](./task-31-validation.md)。
...
任务 32 的类型化消息目录、账户语言与设备临时覆盖、`Intl` 格式化、显式本地机器翻译，以及中英/长文本/缺键浏览器证据记录在 [任务 32 验证记录](./task-32-validation.md)。
...
任务 33 的 Canvas/DOM 双层语义、方向键与焦点闭环、`aria-live`、高对比和低性能列表降级，以及 WCAG 2.2 AA 自动/人工浏览器证据记录在 [任务 33 验证记录](./task-33-validation.md)。
...
任务 34 的幂等发送、未知提交对账、同步缺口、孤儿清理、PWA 版本写入门、六类依赖能力矩阵和自动故障注入证据记录在 [任务 34 验证记录](./task-34-validation.md)。
...
任务 35 的不可信输入边界、严格 CSP、属性测试、远端提示注入证明、五类敏感输出扫描和安全门禁矩阵记录在 [任务 35 验证记录](./task-35-validation.md)。

- [x] 36. 封闭测试发布与验收
  - 自动生成 Windows 测试包、Codex 插件包、Web 版本和单机部署包；macOS 只在维护者自托管 runner 上手动验收。
...
候选 `d794418` 的 M2 矩阵、Windows x86-64 四类制品与签字门已在同一 GitHub Actions 运行通过，并完成下载后独立复验，详见[任务 36 验证记录](task-36-validation.md)。

## M3：公开测试

- [x] 37. 建立双 Homeserver 联邦测试环境
...
候选 `d794418` 的双 Homeserver M3 运行已验证真实到达、回执、Megolm、治理、兼容协商与停机回填；实现与证据分别见[任务 37](task-37-validation.md)和[任务 38](task-38-validation.md)验证记录。
...
数据库、Matrix、内容、30 分钟联邦断网与 Web 生产预算已有真实证据；72 小时活跃 Bridge 常驻及同修订汇总门仍未通过，详见[任务 39 验证记录](task-39-validation.md)。
...
- [x] 41. 完成备份、恢复与数据删除
  ...
  已完成自动备份、隔离 PITR 恢复、保留与回收、幂等账户导出/删除及联邦残留说明，详见 [任务 41 验证记录](task-41-validation.md)。
...
- [ ] 43. 完成签名发布与安全更新
  ...
  工程实现、自动门禁与发布 Runbook 已完成，详见[任务 43 验证记录](task-43-validation.md)。真实受保护发行、离线密钥仪式及任务 36/40 前置证据未完成，因此保持未勾选。
...
- [ ] 44. 完成开源与自托管发行
  ...
  开源发行面、跨平台引导、自托管 Facade、许可证清单、CI 门禁与干净快照验收已完成，详见[任务 44 验证记录](task-44-validation.md)。真实公网 Linux 安装及前置任务 40/42/43 证据未完成，因此保持未勾选。

- [x] 45. 公开测试 Go/No-Go 验收
...
需求 1–15、十类发布门禁、七个开放阻断及解除条件已经形成机器可验证的书面记录；当前正式决定为 **No-Go**，详见[任务 45 Go/No-Go 决策](task-45-go-no-go.md)。完成本任务表示决策已经作出，不表示公开测试获准发布。
...
M0 工程地基、M1 内部纵向切片和 M2 封闭测试已通过验收；M3 的双 Homeserver 联邦与治理兼容也已完成。当前实现侧没有未处理的 M2/M3 功能缺口，任务 39 只等待真实活跃 Bridge 连续运行 72 小时后才能关闭；任务 40–45 的公网部署、故障演练、签名发行、外部复现和最终 Go/No-Go 仍受各自外部证据约束。
