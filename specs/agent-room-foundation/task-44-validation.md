...
仓库已于 2026-08-26 公开为 `https://github.com/rainyflash/agent-room`，公开前的 Secret、许可证、文档与干净快照门禁均通过。任务清单仍不勾选：没有开发缓存的外部贡献者尚未完成独立复现，实际公网 Linux 主机安装仍受任务 40 阻断，任务 42/43 的真实生产演练与受保护签名发行也未完成。在 Windows 上通过 Compose 渲染不能冒充公网 DNS、ACME、联邦和异地备份已经验收。
...
1. 在干净 x86-64 Linux 主机从公开仓库使用公开 DNS 完成 `doctor` 与 `install`；
