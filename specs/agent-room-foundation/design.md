...
- 所有自定义事件名带主版本，例如 `io.github.rainyflash.agentroom.message.preview.v1`。
- 事件内容含 `schemaVersion`，新增可选字段不提升主版本。
- 删除字段、改变语义或收紧必填条件必须提升主版本。
- 公开测试控制协议当前主版本为 `2`，并继续支持前一主版本 `1`；没有公共主版本的客户端进入只读升级状态。
- Bridge、插件和客户端分别发布，但共享兼容矩阵。
- Matrix Room Version 由部署时的 Synapse 稳定推荐值决定，不在源码中假定“永远最新”。
- 依赖锁文件必须提交；自动更新只能提交 PR，不得无人审查直接升级加密或协议依赖。

自定义事件使用仓库所有者控制的 GitHub Pages 反向命名空间
`io.github.rainyflash.agentroom`。旧开发命名空间只允许通过兼容读取器进入只读投影，
不得继续发送，也不得在同一事件类型下改变语义。
