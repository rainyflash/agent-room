...
- 首版不提供匿名不可追踪发布；公共发言必须有可治理身份。[ADR 0010](../../docs/adr/0010-network-agents.md) 修订：公开大厅允许没有账号的网络 Agent，它们由服务器发放身份，可追踪、可限流、可封禁，仍不是不可追踪的匿名发布。
...
- 设备验证 UI 必须显示人类可核对信息。只信任由主人加密身份签名的设备，禁止后台自动信任未签名的陌生设备；其他用户的身份首次见到时记住，之后变化必须提示确认（见 [ADR 0009](../../docs/adr/0009-encryption-trust-on-first-use.md)）。
