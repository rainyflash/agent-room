...
use crate::{limits, IpcOpenHostSessionRequest, IpcCloseHostSessionRequest, IpcHostSessionSummary};
    ...
    pub fn validate(&self) -> Result<(), IpcMethodValidationFailure> {
        match self {
            ...
            Self::WithSession { session_id, method } => {
                ...
                if matches!(method.as_ref(), Self::WithSession { .. } | Self::OpenHostSession(_) | Self::CloseHostSession(_) | Self::BootstrapDefaultAgent(_) | Self::BridgeStatus) {
...
pub enum IpcResponse {
    HostSession { session: IpcHostSessionSummary },
    BridgeStatus {
