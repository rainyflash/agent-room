...
pub(crate) fn validate_session_id(value: &str) -> Result<(), IpcMethodValidationFailure> {
    let parsed = Uuid::parse_str(value)
        .map_err(|_| failure("bridge.ipc.session_id_invalid"))?;
    if parsed.get_version() != Some(Version::SortRand) || parsed.to_string() != value {
...
mod tests {
    ...
    fn 会话包装保留原操作的授权范围且拒绝嵌套包装() {
        ...
        assert_eq!(nested.validate().expect_err("嵌套不得被路由").code(), "bridge.ipc.session_method_invalid");
    }

    #[test]
    fn 会话键不能用路径或名称冒充且拒绝空名称() {
        for session_key in ["../other-agent", "Agent A", "", "01A07063-4799-7a29-88e1-c9f43de239ef"] {
            let request = IpcOpenHostSessionRequest { session_key: session_key.into(), display_name: "调试人物".into() };
            assert!(request.validate().is_err());
        }
        let request = IpcOpenHostSessionRequest { session_key: Uuid::now_v7().to_string(), display_name: " ".into() };
        assert!(request.validate().is_err());
    }
