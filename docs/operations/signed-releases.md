...
日常执行优先使用 [可续跑的发布命令](../release-workflow.md)。以下底层步骤和信任边界继续生效；调度命令负责组织它们，不替代签名、兼容晋级或真实验收。
...
- 候选失败：保留 Draft 与 Actions 证据，不更新渠道入口。若只是草稿创建或上传失败，按候选构建一节恢复原产物；若必须修改应用代码，使用新序号和新标签重新候选。
