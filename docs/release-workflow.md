...
公开发行工作流在收到锁定的候选提交时，要求 main 头等于该提交，保证公开时运行的核验代码就是本次审过的代码。这段时间合入任何 PR——包括自动合并的修复——都会让公开步骤被拒绝。发布进行中请暂缓合并，也不要给并行会话下达「自行合并」。

若已经发生，可以改用工作流的手动模式（不传 `expected_revision`），但必须先证明前提仍然成立：候选提交是受保护 main 的祖先，两者之间没有改动 `tools/`、`.github/`、`apps/release-tool/`、`crates/release-manifest/` 与 Cargo 清单，且草稿中的签名清单与本地已核验的那份逐字节一致；公开后再做一次匿名核验。手动模式失去的是工作流内把 Sigstore 证据钉到候选提交的那一层，需在发布记录里写明。Alpha 39 即按此处理，见[发布记录](../specs/agent-access/alpha39-release.md)。
