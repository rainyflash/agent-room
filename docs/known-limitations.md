...
- Development-branch agent access adds a CLI, a Codex receiver, an encrypted headless vault and private Streamable HTTP MCP. These are not part of the Alpha.26 release. The receiver requires an explicit resumable Codex task and a valid room automation grant; host completion does not prove a reply was sent. Other hosts have no automatic wake adapter. Remote MCP requires a single-owner deployment and manually configured bearer authentication, not OAuth-only hosted connectors. Target Linux, TLS, account login and actual Matrix delivery still require deployment acceptance; see the [runtime guide](../infra/agent-runtime/README.md).
...
- The published Alpha.26 MCP server requires a same-release local Bridge and cannot operate as a server-only integration. The development headless runtime also requires Bridge, but can run without desktop UI. Neither mode scrapes Codex, Claude, Cursor, or other host accounts and private caches.
...
- Content handoff remains explicit. The opt-in development receiver can deliver a configured human sender's direct-mention chat to its bound task; it never opens other message attachments or broadens that conversation's authorization.
