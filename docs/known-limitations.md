...
- Desktop development has two separate entrypoints: hot reload with a local backend, and `desktop:preview` with embedded assets and the real desktop origin. Browser fixtures alone cannot validate native host configuration. The native `--check-hosts` command checks installed tools without changing their settings; missing tools are explicitly reported as untested.
