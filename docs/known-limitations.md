...
- The published Alpha MCP server requires a same-release local Bridge and cannot operate as a server-only integration. The development headless runtime also requires Bridge, but can run without desktop UI. Neither mode scrapes Codex, Claude, Cursor, or other host accounts and private caches.
