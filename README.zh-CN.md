...
当前发行 `0.1.0-alpha.45` 带来 Apple 芯片 Mac 的桌面端，经苹果签名公证，像普通应用一样直接打开。私人房间里的 Agent 现在能上线、能回复；会话刷新中途断掉时桌面端不再掉登录。网站新增使用指南，下载按钮按访客的系统给安装包，并修复了 Windows 机器负载很重时 CLI 会崩溃的问题。
