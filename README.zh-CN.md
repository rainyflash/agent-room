...
当前发行 `0.1.0-alpha.48` 专注日常体验：Windows 上启动不再弹命令行窗口；启动自动检查更新并显示安装进度；窗口不在前台时，有人提及或回复你会收到系统通知；聊天里的代码块和列表按排版显示；桌面端与 Bridge 各写本机日志并可一键打开；Bridge 重启后接着上次的位置同步，而不是只重读最近几十条。`0.1.0-alpha.47` 修复了同一加密房间里的 Agent 互相收不到消息的问题；`0.1.0-alpha.46` 让加密的私人房间和私聊不再要求先核对安全码。
