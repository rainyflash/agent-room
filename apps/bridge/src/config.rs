...
pub(crate) struct BridgeConfig {
    ...
    /// 桌面托管启动时设置：标准输入是桌面持有的管道，读到结束说明桌面已经退出，Bridge 随之有序退出。
...
impl BridgeConfig {
    ...
    fn from_source(source: &impl EnvironmentSource) -> Result<Self, BridgeConfigError> {
        ...
        Ok(Self {
            ...
            exit_with_supervisor: read_bool(
                source,
...
mod tests {
    ...
    fn 最小配置使用安全默认值且不导入第三方资料() {
        ...
        assert!(
            !config.exit_with_supervisor,
            "手动运行或标准输入为空时不能因输入结束而退出"
        );
    ...
    fn 随桌面退出只接受显式布尔开关() {
        ...
        environment
            .0
            .insert("AGENT_ROOM_BRIDGE_EXIT_WITH_SUPERVISOR", "true".to_owned());
        assert!(
            BridgeConfig::from_source(&environment)
                .expect("显式开关有效")
        ...
        environment
            .0
            .insert("AGENT_ROOM_BRIDGE_EXIT_WITH_SUPERVISOR", "yes".to_owned());
        assert!(matches!(
            BridgeConfig::from_source(&environment),
            Err(BridgeConfigError::Invalid {
                name: "AGENT_ROOM_BRIDGE_EXIT_WITH_SUPERVISOR",
                ..
            })
        ));
    }
