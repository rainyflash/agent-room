...
def verify(root: Path, tool: Path, configuration: Path) -> None:
    ...
    installers = [artifact for artifact in artifacts if artifact.kind == "installer" and artifact.platform == "windows-x86_64"]
    receipt = release.load_object(root / "windows-installer-acceptance.json", "installer acceptance")
    installed = receipt.get("installer")
    checks = receipt.get("checks")
    if len(installers) != 1 or not isinstance(installed, dict) or not isinstance(checks, dict):
        raise release.ReleaseFailure("缺少 Windows 安装及运行升级验收。")
    if receipt.get("schemaVersion") != 1 or receipt.get("result") != "passed" or receipt.get("version") != metadata.get("version") or receipt.get("platform") != "windows-x86_64" or installed.get("sha256") != release.sha256_file(installers[0].path) or any(checks.get(key) is not True for key in INSTALLER_CHECKS):
